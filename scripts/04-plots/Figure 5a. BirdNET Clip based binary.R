...
ClipDetections <- list.files('/Volumes/DJC Files/RSOS Benchmarking Data/BirdNETBinaryClipBasedVietnam',
...
for(a in 1: length(ClipDetections)){
  ...
  TempRow$PerformanceFolder <-  str_split_fixed(ClipDetectionsShort[a],pattern = '/',n=7)[,6]
...
ClipDetections <- list.files('/Volumes/DJC Files/RSOS Benchmarking Data/BirdNETMultiClipBasedVietnam',
                             recursive = T,full.names = T)
