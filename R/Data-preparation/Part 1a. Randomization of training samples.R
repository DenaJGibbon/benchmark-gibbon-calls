...
train.signal <- 'data/AcousticData/Jahoo_trainingdata_manual_annotations/Gibbons'
list.train.wavs <- list.files(train.signal,full.names = T)
signal.class <- 'gibbon'
length(list.train.wavs)

train.noise <- 'data/AcousticData/Jahoo_trainingdata_manual_annotations/Noise'
list.noise.wavs <- list.files(train.noise,full.names = T)
noise.class <- 'noise'
length(list.noise.wavs)

output.dir.random <- 'randomization/JahooGibbonClipsRandom/'
