...
for(z in 1:length(PerformanceFolders)){
  ...
  BestF1data.framecrestedgibbonBinary$PerformanceFolder <- basename(PerformanceFolders[[z]])

  TopModelDetectionDF$BinaryLabel <-
    ifelse(TopModelDetectionDF$Class=='gibbon',1,0)

  ROCRpredBirdNETBinary <- ROCR::prediction(predictions = as.numeric(TopModelDetectionDF$Confidence),
  ...
  BestF1data.framecrestedgibbonBinary$auc <- as.numeric(BirdNETBinary_AUC)


  CombinedF1dataBirdNET <- rbind.data.frame(CombinedF1dataBirdNET,BestF1data.framecrestedgibbonBinary)
}
...
#graphics.off()
ggpubr::ggerrorplot(data=CombinedF1dataBirdNET,x='samples',y='auc')+xlab('')+ylab('AUC')+ylim(0,1)
